...
function cardClick(evt) {
  // Get the index of each card when clicked
  var ul = Array.prototype.slice.call(document.getElementsByClassName('deck')[0].children);
  cIndex = ul.indexOf(evt.target);
  console.log('Index is: ' + cIndex);
...
function testCard(tile) {
  ...
  // add card index to openCards array
  console.log(cIndex);
  openCards.push(tileName + ',' + cIndex);
  console.log(openCards);

  // check whether the icons match
  if(openCards.length == 2) {
	// split openCards element and get index from clicked elements
	let o1 = openCards[0].split(',');
	let o2 = openCards[1].split(',');
	// only matchs if index is different, avoiding match to click twice in the same element
	console.log(o1);
	console.log(o2);
  ...
  if(openCards.length == 2 && !matching) {
    ...
    setTimeout(function() {
        openCards.forEach(function(item) {
		  // Fix item getting due to changes on openCards structure
		  let it = item.split(',');
          let shownIcon = document.getElementsByClassName(it[0]);
